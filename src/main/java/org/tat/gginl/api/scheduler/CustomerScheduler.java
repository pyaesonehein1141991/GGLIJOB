...
public class CustomerScheduler {
  ...
  public void createCustomerFolder() throws Exception {
    ...
    if (dateList.size() > 0) {
       ...
       * toCheckSumFile); File checksumFile = new File("CustomerInfoChecksum".concat(".md5"));
       * 
       * objectMapper.writeValue(checksumFile,checksum); String tempDir=
       * fileDir.concat(":\\AceSharedFolder\\CustomerInfo").concat(FileService.getDateToString(new
       * Date()));
       * 
