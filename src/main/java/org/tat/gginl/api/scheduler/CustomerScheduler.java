...
//			writesCsvFromBean(Paths.get(agentsFile.getPath()),agentList);
			...
			writer.close();
//			String tempDir= fileDir.concat("\\CustomersInfo").concat(FileService.getDateToString(new Date()));
			String tempDir= fileDir;
			Path filePath = Paths.get(tempDir.concat("\\Customers.csv"));
			Files.createDirectories(filePath.getParent());
			Files.move(Paths.get(customerFile.getPath()),Paths.get(tempDir.concat("\\Customers.csv")),StandardCopyOption.REPLACE_EXISTING);
